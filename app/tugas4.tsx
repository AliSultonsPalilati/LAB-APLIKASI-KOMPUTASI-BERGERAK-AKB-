...
const fontList: FontConfig[] = [
  { family: 'LatoRegular', name: 'Lato' }, { family: 'RobotoMedium', name: 'Roboto' },
  { family: 'OpenSansRegular', name: 'Open Sans' }, { family: 'PoppinsBold', name: 'Poppins' },
...
export default function Tugas4() {
  ...
  // Fungsi ini HANYA untuk menangani klik tombol
  const cariMahasiswa = () => {
    Keyboard.dismiss();
  ...
  // PERBAIKAN: useEffect HANYA untuk pencarian awal saat komponen dimuat.
  // Logika pencarian awal dimasukkan ke sini agar tidak bergantung pada fungsi di luar.
  useEffect(() => {
    ...
    setData({ sebelum, setelah, target: daftarMhs[idx] });
  }, []); // Dependency array kosong sudah benar karena tidak ada dependensi dari luar.

  return (
    <ScrollView style={s.sv}>
      <Text style={s.header}>Tugas 4: Font & Urutan</Text>
      
      <View style={s.searchBox}>
        <TextInput style={s.input} value={nim} onChangeText={setNim} keyboardType="numeric" />
        <TouchableOpacity style={s.btn} onPress={cariMahasiswa}>
          <Text style={s.btnTxt}>Cari</Text>
        </TouchableOpacity>
      </View>

      {data.target && (
        <View>
          <Text style={s.subhead}>5 Nama Sebelum (Font Statis)</Text>
          {data.sebelum.map((mhs, i) => (
            <View key={mhs.nim} style={s.card}>
              <Text style={[s.name, { fontFamily: fontList[i].family }]}>{mhs.nama}</Text>
              <Text style={s.info}>Font: {fontList[i].name}</Text>
            </View>
          ))}

          <Text style={s.subhead}>Mahasiswa Target</Text>
          <View style={s.targetCard}>
            <Text style={s.targetName}>{data.target.nama}</Text>
            <Text style={s.targetNim}>{data.target.nim}</Text>
          </View>

          <Text style={s.subhead}>5 Nama Setelah (Font Variabel)</Text>
          {data.setelah.map((mhs, i) => {
            const fontIdx = i + 5;
            return (
              <View key={mhs.nim} style={s.card}>
                <Text style={[s.name, { fontFamily: fontList[fontIdx].family, fontWeight: fontList[fontIdx].weight }]}>
                  {mhs.nama}
                </Text>
                <Text style={s.info}>Font: {fontList[fontIdx].name} (Wt: {fontList[fontIdx].weight})</Text>
              </View>
            );
          })}
        </View>
